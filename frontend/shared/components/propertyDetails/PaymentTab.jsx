...
import {
  Box, Paper, Typography, Grid, Card, CardContent, Alert
} from '@mui/material'
import {
  Payment, Upload, CheckCircle, Schedule, TrendingUp, Description, Info
...
// usePaymentTabColumnsFn se inyecta como prop — cada app pasa la suya
const PaymentTab = ({ apartmentDetails, payloads, loadingPayloads, onPaymentUploaded, user, usePaymentTabColumnsFn }) => {
  ...
  const {
    ...
    handlePaymentUpload,
  } = usePaymentTab({ resourceType: 'apartment', resourceId: apartmentDetails?._id, user })

  const columns = usePaymentTabColumnsFn?.({ t }) ?? []

  if (!apartmentDetails) {
    return <Box p={3}>{t('myApartment:loading', 'Loading...')}</Box>
  }

  const summaryCards = [
    {
      label: t('myApartment:totalPaid', 'Total Paid'),
      value: `$${apartmentDetails.payment?.totalPaid?.toLocaleString() ?? '0'}`,
      color: '#8CA551',
      icon: <CheckCircle />,
    },
    {
      label: t('myApartment:pendingAmount', 'Pending Amount'),
      value: `$${apartmentDetails.payment?.pending?.toLocaleString() ?? '0'}`,
      color: '#E5863C',
      icon: <Schedule />,
    },
    {
      label: t('myApartment:paymentProgress', 'Payment Progress'),
      value: `${Math.round(apartmentDetails.payment?.progress ?? 0)}%`,
      color: '#333F1F',
      icon: <TrendingUp />,
    },
  ]
        ...
        >
          <Box display="flex" alignItems="center" gap={{ xs: 1.5, sm: 2 }}>
            ...
            <Box>
              ...
              >
                {t('myApartment:paymentTab', 'Payment Status')}
              </Typography>
              <Typography
                variant="body2"
                sx={{
              ...
              >
                {t('myApartment:managePayments', 'Manage and track your payment history')}
              </Typography>
            </Box>
          </Box>
          ...
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
            ...
            >
              {t('myApartment:uploadPayment', 'Upload Payment')}
            </PrimaryButton>
          </motion.div>
        </Box>
        ...
        <Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
            ...
            <Box>
              ...
              >
                {t('myApartment:paymentHistory', 'Payment History')}
              </Typography>
              <Typography
                variant="caption"
                sx={{
              ...
              >
                {t('myApartment:transactions', { count: payloads.length })}
              </Typography>
            </Box>
          </Box>
          ...
          <DataTable
            ...
            emptyState={
              ...
              >
                {t('myApartment:noPayments', 'No payment transactions yet. Click "Upload Payment" to add your first payment.')}
              </Alert>
            }
            stickyHeader
            maxHeight={400}
