...
const QuickActionsPanel = ({ actions, title, subtitle }) => {
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
        {actions.map((action, index) => (
          <motion.div
            ...
            whileTap={{ scale: 0.98 }}
          >
            <Card onClick={action.onClick} elevation={0} sx={{
              cursor: 'pointer', borderRadius: 3,
              border: '1px solid transparent', bgcolor: 'white',
              boxShadow: '0 2px 8px rgba(0,0,0,0.04)', transition: 'all 0.3s',
              position: 'relative', overflow: 'hidden',
              '&::before': {
                content: '""', position: 'absolute', left: 0, top: 0, bottom: 0,
                width: 3, bgcolor: action.color, transform: 'scaleY(0)', transition: 'transform 0.3s'
              },
              '&:hover': {
                borderColor: action.color,
                boxShadow: `0 8px 20px ${action.color}20`,
                '&::before': { transform: 'scaleY(1)' },
                '& .action-icon-container': { transform: 'scale(1.1) rotate(5deg)', bgcolor: action.color, '& svg': { color: 'white' } },
                '& .arrow-container': { transform: 'translateX(4px)', bgcolor: action.color, color: 'white' }
              }
            }}>
              <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                <Box display="flex" alignItems="center" gap={1.5}>
                  <Box className="action-icon-container" sx={{
                    width: 48, height: 48, borderRadius: 2.5, bgcolor: action.bgColor,
                  ...
                  <Box className="arrow-container" sx={{
                    width: 32, height: 32, borderRadius: '50%', bgcolor: action.bgColor,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    color: action.color, fontSize: '1.1rem', fontWeight: 'bold', transition: 'all 0.3s'
                  }}>
                    ›
                  </Box>
                </Box>
