    ...
    <Paper elevation={0} sx={paperSx}>
      ...
      <Box sx={headerSx}>
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <Typography sx={sectionLabelSx}>
            {t('quote:customizeStep', '04 CUSTOMIZE YOUR APARTMENT')}
          </Typography>
        </Box>
        <Chip
          label={`${t('quote:apartmentShort', 'Apt')} ${selectedApartment?.apartmentNumber || t('common:notAvailable', 'N/A')}`}
          size="small"
          sx={aptChipSx}
        />
      </Box>
      ...
      {model && (
        <Box sx={{ p: 2, bgcolor: theme.palette.cardBg || '#fafafa' }}>
          <Typography sx={sectionSubLabelSx} mb={1}>
            {t('quote:apartmentModel', 'Apartment Model')}
          </Typography>
      ...
      {/* Package Selection */}
      <Box sx={{ p: 2 }}>
        <Typography sx={sectionSubLabelSx} mb={2}>
          {t('quote:selectInteriorPackage', 'Select Interior Package')}
        </Typography>
          ...
          <Box sx={{ p: 2 }}>
            ...
            >
              <img
                ...
                alt={t('quote:previewImage', 'Preview {{idx}}', { idx: previewIdx + 1 })}
            ...
            {renders.length > 1 && (
              <Box sx={{ display: 'flex', gap: 1, mt: 1, overflowX: 'auto' }}>
                {renders.map((url, i) => (
                  ...
                  >
                    <img src={url} alt={t('quote:previewThumb', 'Thumb {{idx}}', { idx: i + 1 })} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </Box>
                ))}
              </Box>
            )}
