...
const MasterPlanPage = ({ projectId }) => {
  ...
  const handleSave = async (updates) => {
    try {
      ...
      alert(t('updateMasterPlan', 'Master plan saved successfully!'))
      setEditorOpen(false)
      await fetchMasterPlan(projectId)
    } catch (error) {
      alert(t('saveError', { error: error.message }, 'Error saving master plan: {{error}}'))
    }
  }
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#fafafa', p: { xs: 2, sm: 3 } }}>
      <Container maxWidth="xl">
        <PageHeader
          icon={Map}
          title={t('title', 'Master Plan Manager')}
          subtitle={t('subtitle', 'Manage inventory, exterior amenities, and master plan tour')}
          actionButton={{
            label: t('updateMasterPlan', 'Edit Master Plan'),
            onClick: handleOpenEditor,
            icon: <Map />,
            tooltip: t('updateMasterPlan', 'Edit master plan polygons'),
            disabled: loading
          }}
        />
          ...
          <Box sx={{ mt: 3, mb: 4 }}>
            <Box mb={3}>
              <Box display="flex" alignItems="center" gap={1.5} mb={1}>
                ...
                >
                  {t('overview', 'Master Plan Overview')}
                </Typography>
              </Box>
              ...
              >
                {t('overviewDescription', 'View all building polygons on the master plan. Click "Edit Master Plan" to modify.')}
              </Typography>
            </Box>
            ...
            >
              ...
              >
                ...
                >
                  {t('showingBuildings', {
                    count: previewPolygons.length
                  }, 'Showing {{count}} building', { count: previewPolygons.length })}
                </Typography>
                ...
                <Box display="flex" gap={2} flexWrap="wrap">
                  ...
                  {previewPolygons.length > 5 && (
                    ...
                    >
                      {t('moreBuildings', { count: previewPolygons.length - 5 }, '+{{count}} more')}
                    </Typography>
                  )}
                </Box>
              </Box>
