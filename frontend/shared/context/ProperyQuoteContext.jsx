...
export const PropertyQuoteProvider = ({ projectSlug, projectId, children }) => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // ─────────────────────────────────────────────────────────────────────────
  // Recalculate financials when apartment, type, or percentages change
  // ─────────────────────────────────────────────────────────────────────────
  const calculateFinancials = useCallback(() => {
    ...
    // ✅ CORREGIDO: El upgradePrice es el COSTO ADICIONAL, no el precio total.
    const isUpgrade = apartmentType === 'upgrade' || selectedRenderType === 'upgrade'
    
    const basePrice = selectedApartment.price || 0
    const upgradeExtra = isUpgrade ? (selectedApartment.upgradePrice || 0) : 0
    
    // Sumamos el extra solo si es upgrade. Si no, el extra es 0.
    const listPrice = basePrice + upgradeExtra

    const calculated = propertyService.calculateApartmentFinancials({
      listPrice:                  listPrice, // ✅ Ahora sí: 4000 + 1500 = 5500
    ...
    setFinancials(prev => ({ 
      ...prev, 
      ...calculated,
      listPrice // ✅ Asegurar que el contexto guarde el listPrice correcto
    }))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    selectedApartment,
    apartmentType,        // ✅ AGREGADO: Para recalcular cuando cambia el paquete
    selectedRenderType,   // ✅ AGREGADO: Para recalcular cuando cambia el render type
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    selectedApartment,
    apartmentType,        // ✅ AGREGADO
    selectedRenderType,   // ✅ AGREGADO
    financials.discountPercent,
    financials.downPaymentPercent,
    financials.initialDownPaymentPercent,
    financials.monthlyPaymentPercent,
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // NEW: Get quote without creating apartment
  // ─────────────────────────────────────────────────────────────────────────
  const getQuote = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ PropertyQuoteContext – getQuote:', error)
      setError(error.message)
      throw error
    }
  }, [selectedApartment, financials.initialDownPayment, selectedRenderType])
