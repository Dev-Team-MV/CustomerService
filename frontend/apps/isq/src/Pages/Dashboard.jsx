// /Users/oficina/MV-CRM/CustomerService/frontend/apps/isq/src/pages/Dashboard.jsx
...
import { getIsqDashboardTourSteps, isqDashboardTourConfig } from '../tours/modules/dashboardTour'
...
// ─── Payload Row (Estilizado con colores del theme de ISQ) ───────────────────
const PayloadRow = ({ payload, navigate, t, C }) => {
...
const Dashboard = () => {
  ...
  // ✅ Colores dinámicos basados en el theme de ISQ
  const C = {
  ...
  const dashboardSteps = getIsqDashboardTourSteps(tCommon)
  ...
  return (
    ...
    <Box id="isq-dashboard-container" sx={{ minHeight: '100vh', bgcolor: C.bg }}>
      
      {/* ✅ Botón del Tour */}
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', px: { xs: 3, md: 6 }, pt: 2 }}>
        <TourButton 
          tourId={isqDashboardTourConfig.id}
          steps={dashboardSteps}
          label={tCommon('tour.isqDashboard.button', 'Guía del Dashboard')}
          options={tourOptions}
        />
      </Box>

      {/* ── HEADER (Estilo Lakewood, colores ISQ) ── */}
        ...
        <Box sx={{ px: { xs: 3, md: 6 }, pt: { xs: 4, md: 5 }, pb: 3 }}>
          ...
          <Box display="flex" alignItems="center" gap={1.5} mt={1.5}>
            <Typography variant="body2" sx={{ color: C.gray, fontFamily: C.fontFamily, fontSize: '0.9rem' }}>
              {t('subtitle', 'Panel de control ISQ')}
            </Typography>
            <Chip
      ...
      <Box id="isq-layout-intro-trigger" sx={{ height: 1, width: 1 }} />
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }}>
        ...
        <Box id="isq-map-section">
          ...
          >
            <AmenitiesMap
              mapImage={null} // ✅ Específico de ISQ
              ...
              {actions.map((action, index) => {
                ...
                return (
                  <Box key={index} sx={{ display: 'flex', alignItems: 'center', gap: { xs: 2, md: 3 }, borderLeft: { xs: 'none', sm: col === 1 ? `1px solid ${C.border}` : 'none' }, borderBottom: isLastRow ? 'none' : `1px solid ${C.border}`, pl: { xs: 0, sm: col === 1 ? 5 : 0 }, pr: { xs: 0, sm: col === 0 ? 5 : 0 }, py: 4 }}>
                    ...
                    <Box component={motion.div} whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.95 }} onClick={action.onClick} sx={{ width: { xs: 52, md: 60 }, height: { xs: 52, md: 60 }, bgcolor: C.bgLight, borderRadius: 2.5, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, color: C.primary, fontSize: '1.3rem', transition: 'background 0.2s', '&:hover': { bgcolor: C.primary, color: 'white' } }}>
                      ↗
                    </Box>
                  </Box>
                )
      ...
      {isAdmin && payloads?.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.5 }}>
          ...
          <Box id="isq-recent-payloads" sx={{ px: { xs: 3, md: 6 }, py: 4 }}>
            <PageSection
            ...
            >
              <Box sx={{ py: 2 }}>
                {payloads.map((payload) => (
                  <PayloadRow key={payload._id} payload={payload} navigate={navigate} t={t} C={C} />
                ))}
              </Box>
            </PageSection>
          </Box>
