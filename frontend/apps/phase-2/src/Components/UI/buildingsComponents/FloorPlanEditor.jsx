...
import {
  ...
  Undo, Redo, Save, CheckCircle, NavigateBefore, NavigateNext
  ...
  useEffect(() => {
    if (floorPlans.length > 0) {
      ...
      floorPlans.forEach(fp => {
        ...
        initialPolygons[floorKey] = fp.polygons || []
        initialHistory[floorKey] = [fp.polygons || []]
  ...
  const handleSave = () => {
    const allFloorData = floorPlans.map(fp => {
      ...
      return {
        ...
        url: fp.url,
        polygons: floorPolygons[floorKey] || []
      }
    })
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', height: 'calc(100% - 140px)' }}>
        ...
        >
          ...
          >
            ...
            <ToggleButton value="polygon" sx={{ py: 1.5 }}>
              <Tooltip title="Custom Polygon" placement="right">
                <Timeline />
              </Tooltip>
            </ToggleButton>
          </ToggleButtonGroup>
        ...
        <Box sx={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', bgcolor: '#f5f5f5', position: 'relative' }}>
          {drawMode === 'polygon' && currentPoints.length > 0 && (
            ...
            >
              Click to add points. {currentPoints.length / 2} points added.
            </Alert>
          )}
          ...
          <Stage
            ...
            style={{ border: `2px solid ${theme.palette.divider}`, backgroundColor: '#fff' }}
          >
            <Layer>
              {image && (
                <KonvaImage
        ...
        >
                  ...
                  <ListItemText
                    ...
                    secondary={
                      polygon.apartmentModel 
                        ? apartmentModels.find(m => m._id === polygon.apartmentModel)?.name || 'Unknown'
                        : 'Not assigned'
                    }
                    primaryTypographyProps={{ fontFamily: '"Poppins", sans-serif', fontSize: '0.9rem' }}
          ...
          {selectedPolygon && (
            <Box sx={{ p: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 700, fontFamily: '"Poppins", sans-serif', mb: 2 }}>
                Polygon Settings
              </Typography>
              <TextField
                fullWidth
              ...
              >
                ...
                {apartmentModels.map((model) => (
                  <MenuItem key={model._id} value={model._id}>
                    {model.name} ({model.bedrooms}BR/{model.bathrooms}BA)
                  </MenuItem>
                ))}
              </TextField>
            </Box>
      ...
      <Box sx={{ 
        ...
        <Typography variant="caption" sx={{ color: theme.palette.text.secondary, fontFamily: '"Poppins", sans-serif' }}>
          Zoom: {Math.round(scale * 100)}% | Total Polygons: {totalPolygons} | Total Assigned: {totalAssigned}
        </Typography>
