...
import { useApartments } from '@shared/hooks/useApartments'
...
const Properties = () => {
  ...
  const { assigned, loading, refresh } = useApartments(null, PROJECT_ID)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [modelsRes, usersRes, buildingsRes] = await Promise.all([
          buildingService.getApartmentModels(),
          api.get('/users'),
          buildingService.getBuildingsByProject(PROJECT_ID)
        ])
  ...
  const columns = usePropertyColumns({
    ...
    onDelete: (row) => alert(t('actions.deleteProperty', { name: row.name })),
    onOpenContracts: (row) => {
      setSelectedApartment(row.raw)
