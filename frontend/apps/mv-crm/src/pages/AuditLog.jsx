...
import { getAuditLogTourSteps, auditLogTourConfig } from '../tours/modules/auditLogTour'
...
const AuditLog = () => {
  ...
  const tourSteps = getAuditLogTourSteps(tCommon)
  ...
  // ✅ LÓGICA DEL TOUR (Índices actualizados para el recorrido detallado del Drawer)
  const handleTourNextClick = (driverObj) => {
    ...
    // ==========================================
    // PASO 13: Abrir Drawer de Detalles
    // ==========================================
    if (currentIndex === 13) {
      if (logs.length > 0) {
        console.log('✅ Abriendo drawer de detalles para el primer log...')
        handleOpenDetail(logs[0])
        pauseTour()
        
        const checkDrawer = setInterval(() => {
          const drawerElement = document.getElementById('audit-detail-drawer')
          if (drawerElement) {
            clearInterval(checkDrawer)
            console.log('✅ Drawer encontrado en el DOM. Reanudando tour en el paso 14...')
            setTimeout(() => {
              // ✅ Usamos resumeTour para continuar en el primer paso del drawer
              resumeTour(14, tourSteps, tourOptionsRef.current)
            }, 300)
          }
        }, 150)
      } else {
        console.warn('⚠️ No hay logs para mostrar el drawer. Avanzando...')
        driverObj.moveNext()
      }
      return
    }
    ...
    if (currentIndex === 18) {
      ...
      if (closeBtn) {
        console.log('✅ Cerrando drawer de detalles...')
        closeBtn.click()
        setTimeout(() => {
          driverObj.moveNext() // Avanza al paso 19 (Finish)
        }, 400)
      } else {
        handleCloseDetail()
    ...
    // Para el resto de pasos (0-12, 14-17, 19), avance normal
    driverObj.moveNext()
  }
  ...
  const tourOptions = {
    ...
    onDestroy: () => {
      console.log('🛑 Tour de auditoría destruido, limpiando modo tour')
      setIsTourMode(false)
    }
  }
  ...
  return (
    <PageLayout title={t('title', 'Audit Log')} titleBold={t('titleBold', 'CRM')} topbarLabel={t('topbarLabel', 'Audit')} subtitle={t('subtitle', 'System activity records')}>
      ...
      <Box id="audit-log-page-container">
        
        {/* ✅ Botón del Tour */}
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', p: 2, pb: 0 }}>
          <TourButton 
            tourId={auditLogTourConfig.id}
            steps={tourSteps}
            label={tCommon('tour.auditLog.button', 'Ver guía de Auditoría')}
            options={tourOptions}
          />
        </Box>

        {/* ✅ ID: Filtros */}
        <Paper id="audit-log-filters" elevation={0} sx={{ p: 3, mb: 3, borderRadius: 0, border: '1px solid #e0e0e0', bgcolor: '#fafafa' }}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6} md={3}>
              {/* ✅ ID: Filtro de Entidad */}
              <Box id="audit-filter-entity">
                <FormControl size="small" fullWidth>
                  <InputLabel sx={{fontFamily: '"Courier New", monospace'}}>{t('filters.entity', 'Entity')}</InputLabel>
            ...
            <Grid item xs={12} sm={6} md={3}>
              {/* ✅ ID: Filtro de Proyecto */}
              <Box id="audit-filter-project">
                <FormControl size="small" fullWidth>
                  <InputLabel sx={{fontFamily: '"Courier New", monospace'}}>{t('filters.project', 'Project')}</InputLabel>
            ...
            <Grid item xs={12} sm={6} md={3}>
              {/* ✅ ID: Filtro de Acción */}
              <Box id="audit-filter-action">
                <FormControl size="small" fullWidth>
                  <InputLabel sx={{fontFamily: '"Courier New", monospace'}}>{t('filters.action', 'Action')}</InputLabel>
            ...
            <Grid item xs={12} sm={6} md={3}>
              {/* ✅ ID: Filtro de Usuario */}
              <Box id="audit-filter-user">
                <FormControl size="small" fullWidth>
                  <InputLabel sx={{fontFamily: '"Courier New", monospace'}}>{t('filters.user', 'User')}</InputLabel>
            ...
            {/* ✅ ID: Filtros de Fechas */}
            <Grid item xs={12} sm={6} md={3}>
              <Box id="audit-filter-dates">
        ...
        <Box id="audit-log-finish" sx={{ height: 1 }} />
      </Box>
      
      {/* ✅ Drawer de Detalles con IDs */}
      <AuditLogDetailDrawer 
        log={selectedLog} 
        open={drawerOpen} 
        onClose={handleCloseDetail} 
        t={t}
        tourId="audit-detail-drawer"
        closeBtnId="audit-detail-close"
      />
    </PageLayout>
  )
}
