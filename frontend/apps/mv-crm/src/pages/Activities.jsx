...
export default function Activities() {
  ...
  const activityIdFromUrl = searchParams.get('activityId') // ✅ NUEVO
  ...
  // ✅ NUEVO: Cargar actividad desde URL si existe activityId
  useEffect(() => {
  ...
  const loadActivityFromUrl = async (activityId) => {
    try {
      ...
      for (const column of columns) {
        ...
        if (activity) {
          setDetailsActivity(activity)
          // ✅ OPCIONAL: Limpiar el query parameter después de abrir
      ...
      // Si no está en las columnas cargadas, buscar directamente
  ...
  const handleViewActivity = (activity) => {
    setDetailsActivity(activity)
    // ✅ OPCIONAL: Actualizar URL con el activityId
    // searchParams.set('activityId', activity._id)
    // setSearchParams(searchParams)
  }
    ...
    <PageLayout>
      <Box sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <ActivityDetails
          ...
          onClose={() => {
            setDetailsActivity(null)
            // ✅ OPCIONAL: Limpiar query parameter al cerrar
