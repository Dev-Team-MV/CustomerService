...
// import { Box, Button, Typography, Divider, LinearProgress, Avatar } from '@mui/material'
...
//           status: ['Active', 'Planning', 'Review'][Math.floor(Math.random() * 3)]
//         }))
//         setProjects(enriched)
//         if (enriched.length > 0) setSelectedProject(enriched[0])
//       })
//       .catch(() => setProjects([]))
...
//         { label: t('sidebarStats.users'), value: totalUsers }
//       ]}
//     >
...
//         { label: t('metrics.revenue'), value: Math.round(totalRevenue / 1000), prefix: '$', suffix: 'K' },
//       ]} />

//       {/* ── Split view ── */}
//       <Box sx={{ display: 'flex', gap: 3, alignItems: 'flex-start', flexWrap: { xs: 'wrap', lg: 'nowrap' } }}>
//         {/* Left: project list with scroll */}
//         <Box sx={{
//           flex: '0 0 320px',
//           minWidth: 0,
...
//             {loading
//               ? [0, 1, 2].map(i => (
//                   <Box key={i} sx={{ border: '1px solid #f0f0f0', p: '18px 20px', background: '#fff' }}>
...
//                     {[
//                       { label: t('metrics.totalUsers'),  value: selectedProject.users, prefix: '',  suffix: '', sub: t('metrics.thisMonth', { growth: selectedProject.growth ?? 0 }) },
//                       { label: t('metrics.revenue'),     value: Math.round(selectedProject.revenue / 1000), prefix: '$', suffix: 'K', sub: t('metrics.monthlyRecurring') },
//                       { label: t('metrics.performance'), value: selectedProject.tasks > 0 ? Math.round((selectedProject.completedTasks / selectedProject.tasks) * 100) : 0, prefix: '',  suffix: '%', sub: `${selectedProject.completedTasks}/${selectedProject.tasks} ${t('metrics.tasks')}` },
//                       { label: t('metrics.tasks'),       value: selectedProject.completedTasks, prefix: '',  suffix: `/${selectedProject.tasks}`, sub: t('metrics.complete', { percent: selectedProject.tasks > 0 ? Math.round((selectedProject.completedTasks / selectedProject.tasks) * 100) : 0 }) },
//                     ].map((m, i) => (
//                       <Box key={m.label} sx={{
//                         p: '24px 28px',
...
//                           <Counter to={m.value} prefix={m.prefix} suffix={m.suffix} duration={1.1} />
//                         </Typography>
//                         {m.label === t('metrics.performance') && (
//                           <LinearProgress variant="determinate" value={m.value} sx={{ height: 2, borderRadius: 0, bgcolor: '#f0f0f0', mb: 1, '& .MuiLinearProgress-bar': { bgcolor: '#000', borderRadius: 0 } }} />
//                         )}
//                         {m.label === t('metrics.totalUsers') && selectedProject.growth >= 0 && (
//                           <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.5, px: 1, py: 0.3, border: '1px solid #e0e0e0', mb: 0.5 }}>
//                             <TrendingUp sx={{ fontSize: 10, color: '#000' }} />
//                             <Typography sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.58rem', color: '#000', letterSpacing: '1px' }}>
//                               +{selectedProject.growth}%
//                             </Typography>
//                           </Box>
//                         )}
//                         <Typography sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.58rem', color: '#bbb', letterSpacing: '0.5px' }}>
//                           {m.sub}
//                         </Typography>
//                       </Box>
//                     ))}
...
//                       sx={{ p: '20px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#000', cursor: 'pointer', transition: 'background 0.2s' }}
//                     >
