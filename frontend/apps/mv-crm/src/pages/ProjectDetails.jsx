...
import { getProjectDetailsTourSteps, projectDetailsTourConfig } from '../tours/modules/projectDetailsTour'
...
export default function ProjectDetails() {
  ...
  const tourSteps = getProjectDetailsTourSteps(tCommon)

  useEffect(() => {
    const shouldStartTour = sessionStorage.getItem('startProjectDetailsTour')
    if (shouldStartTour === 'true' && !loading && project) {
      sessionStorage.removeItem('startProjectDetailsTour') // Limpiar bandera inmediatamente
      
      setTimeout(() => {
        console.log('🚀 Iniciando tour de detalles del proyecto...')
        startTour(projectDetailsTourConfig.id, tourSteps, {
          onNextClick: (driver) => {
            const currentStepIndex = driver.getActiveIndex()
            console.log('🔍 Tour ProjectDetails - Paso actual:', currentStepIndex)
            
            // ✅ Paso 5: Navegar a la pestaña de Documentos
            if (currentStepIndex === 5) {
              const documentsTab = document.getElementById('project-documents-tab')
              if (documentsTab) {
                console.log('✅ Navegando a pestaña de Documentos...')
                documentsTab.click()
                setTimeout(() => {
                  driver.moveNext()
                }, 400)
            ...
            // ✅ Paso 8: Navegar a la pestaña de Inventario
            if (currentStepIndex === 8) {
              const inventoryTab = document.getElementById('project-inventory-tab')
              if (inventoryTab) {
                console.log('✅ Navegando a pestaña de Inventario...')
                inventoryTab.click()
                setTimeout(() => {
                  driver.moveNext()
                }, 400)
          ...
          onDestroy: () => {
            console.log('🛑 Tour de detalles de proyecto finalizado')
          }
        })
      }, 600) // Delay para asegurar que el DOM esté completamente renderizado
    }
  }, [loading, project, startTour, tourSteps])
  ...
  return (
    ...
    >
      {/* ✅ ID: Contenedor Principal del Tour */}
      <Box id="project-details-page-container">
        
        {/* ── HERO ── */}
        {/* ✅ ID: Sección Hero */}
        <Box id="project-details-hero" sx={{
        ...
        {/* ── TABS PRINCIPALES ── */}
        {/* ✅ ID: Pestañas Principales */}
        <Box id="project-details-main-tabs" sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
          <Tabs
            value={activeTab}
          ...
          >
            ...
            {/* ✅ IDs agregados a las pestañas individuales para el tour */}
            <Tab id="project-documents-tab" icon={<Description sx={{ mr: 1, fontSize: 18 }} />} iconPosition="start" label={t('tab.documents', 'Documentos')} />
            <Tab id="project-inventory-tab" icon={<Map sx={{ mr: 1, fontSize: 18 }} />} iconPosition="start" label={t('tab.inventory', 'Inventario')} />
        ...
        <TabPanel value={activeTab} index={0}>
          ...
          <Grid container spacing={4}>
            {/* ✅ ID: Sección de Información General */}
            <Grid item xs={12} md={7} id="project-details-info-section">
              <Stack spacing={3}>
            ...
            {/* ✅ ID: Sección de Galería */}
            <Grid item xs={12} md={5} id="project-details-gallery">
              <Stack spacing={3}>
        ...
        <Box id="project-details-finish" sx={{ height: 1, mt: 4 }} />
