...
import {
  ...
  CircularProgress, Alert, Divider, Switch, FormControlLabel, Chip, Grid
} from '@mui/material'
...
const AutomationBuilder = ({ open, onClose, automation = null, onSave, onDelete, onTest, projects = [], agents = [] }) => {
  ...
  const handleSubmit = async () => {
    if (!formData.name) return setError('El nombre es obligatorio')
    if (!formData.trigger) return setError('El trigger es obligatorio')
    if (!formData.action) return setError('La acción es obligatoria')
    if (formData.action === 'send_sms' && !formData.actionPayload.templateId && !formData.actionPayload.message) return setError('Debes seleccionar un template o escribir un mensaje')
    if (formData.action === 'create_activity' && !formData.actionPayload.title) return setError('El título de la actividad es obligatorio')
    if (formData.action === 'notify_agent' && !formData.actionPayload.assignedTo) return setError('Debes seleccionar un asesor para notificar')
  ...
  const handleDelete = async () => {
    ...
    catch (err) { setError(err.response?.data?.message || err.message || 'Error al eliminar') }
    finally { setLoading(false) }
  }
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (result.matched) {
        ...
        if (result.result?.success) alert(`✅ Test exitoso\n\n👤 Cliente: ${clientName}`)
        else alert(`⚠️ Match encontrado pero error:\n\n❌ ${result.result?.error}\n👤 Cliente: ${clientName}`)
      } else {
        alert('⚠️ No se encontraron coincidencias para los criterios actuales.')
      }
    } catch (err) { setError(err.response?.data?.message || err.message || 'Error al probar') }
    finally { setLoading(false) }
  }
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 0, border: '1px solid #ececec' } }}>
      ...
      <DialogContent sx={{ p: 0 }}>
        ...
        <Box display="flex" flexDirection="column" gap={2.5} sx={{ p: 3 }}>
          ...
          <FormControlLabel control={<Switch checked={formData.isActive} onChange={(e) => handleChange('isActive', e.target.checked)} sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: '#4caf50' }, '& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track': { backgroundColor: '#4caf50' } }} />} label={<Typography sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.75rem', color: formData.isActive ? '#4caf50' : '#888' }}>{formData.isActive ? 'Activa' : 'Inactiva'}</Typography>} />
          <Divider />
          ...
          <Box>
            ...
            <FormControl size="small" fullWidth required>
              <Select value={formData.trigger} onChange={(e) => handleChange('trigger', e.target.value)} sx={{ ...inputSx, width: '100%' }}>
                {TRIGGERS.map(trigger => (<MenuItem key={trigger.value} value={trigger.value} sx={menuItemSx}><Box display="flex" alignItems="center" gap={1}><Typography>{trigger.icon}</Typography>{trigger.label}</Box></MenuItem>))}
              </Select>
            </FormControl>
          </Box>

          <Box>
            <Typography sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.7rem', color: '#888', letterSpacing: '1px', textTransform: 'uppercase', mb: 1 }}>Condiciones (opcional)</Typography>
            <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} gap={2}>
              {formData.trigger === 'lead_stage_changed' && (
                <FormControl size="small" fullWidth>
                  <InputLabel>Stage específico</InputLabel>
                  <Select value={formData.condition.stage} onChange={(e) => handleConditionChange('stage', e.target.value)} label="Stage específico" sx={{ ...inputSx, width: '100%' }}>
                    <MenuItem value="" sx={menuItemSx}>Cualquier stage</MenuItem>
                    {['nuevo', 'contactado', 'visita_agendada', 'propuesta', 'vendido', 'perdido'].map(s => (<MenuItem key={s} value={s} sx={menuItemSx}>{s.charAt(0).toUpperCase() + s.slice(1).replace('_', ' ')}</MenuItem>))}
                  </Select>
                </FormControl>
              )}
              ...
              {formData.trigger === 'inactivity_7days' && (
                <TextField label="Días de inactividad" type="number" value={formData.condition.daysInactive} onChange={(e) => handleConditionChange('daysInactive', parseInt(e.target.value) || 7)} fullWidth inputProps={{ min: 1 }} sx={inputSx} />
              )}
            </Box>
              <Box sx={{ flex: 1, mt: 2 }}>
                <ProjectSelector
                  ...
                  includeGlobal={true}
                  globalLabel="Cualquier proyecto"
                  fullWidth
                  size="small"
                />
              </Box>
          ...
          <Box>
            ...
            <FormControl size="small" fullWidth required>
              <Select value={formData.action} onChange={(e) => handleChange('action', e.target.value)} sx={{ ...inputSx, width: '100%' }}>
                {ACTIONS.map(action => (<MenuItem key={action.value} value={action.value} sx={menuItemSx}><Box display="flex" alignItems="center" gap={1}><Typography>{action.icon}</Typography>{action.label}</Box></MenuItem>))}
              </Select>
            </FormControl>
          </Box>

          <Box>
            <Typography sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.7rem', color: '#888', letterSpacing: '1px', textTransform: 'uppercase', mb: 1 }}>Configuración de acción</Typography>
            <Box display="flex" flexDirection="column" gap={2}>
              {formData.action === 'send_sms' && (
                <>
                  {selectedProjectId && (
                    <Box sx={{ p: 1.5, bgcolor: '#e8f5e9', borderRadius: 0, border: '1px solid #4caf50', display: 'flex', alignItems: 'center', gap: 1 }}>
                      ...
                      <Typography variant="caption" sx={{ color: '#2e7d32', fontFamily: '"Courier New", monospace', fontSize: '0.7rem', fontWeight: 600 }}>
                        Vinculada al proyecto: <strong>{selectedProjectName}</strong>
                      </Typography>
                    </Box>
                  )}
                  <FormControl size="small" fullWidth disabled={loadingTemplates}>
                    <InputLabel>Template</InputLabel>
                    <Select value={formData.actionPayload.templateId} onChange={(e) => handleActionPayloadChange('templateId', e.target.value)} label="Template" sx={{ ...inputSx, width: '100%' }}>
                      {loadingTemplates ? (<MenuItem value="" disabled><CircularProgress size={20} sx={{ mr: 1 }} />Cargando...</MenuItem>) : templates.length === 0 ? (<MenuItem value="" disabled>No hay plantillas</MenuItem>) : (
                        templates.map(template => (
                          <MenuItem key={template._id} value={template._id} sx={menuItemSx}>
                            <Box display="flex" alignItems="center" gap={1} width="100%">
                              <Typography sx={{ flex: 1 }}>{template.name}</Typography>
                              <Chip label={template.projectId ? 'proyecto' : 'global'} size="small" sx={{ bgcolor: template.projectId ? '#e3f2fd' : '#f5f5f5', color: template.projectId ? '#1976d2' : '#666', fontSize: '0.6rem', height: 18, borderRadius: 0 }} />
                            </Box>
                          </MenuItem>
                        ))
                      )}
                  ...
                  <Box ref={messageRef}>
                    <TextField label="Mensaje personalizado" value={formData.actionPayload.message} onChange={(e) => handleActionPayloadChange('message', e.target.value)} fullWidth multiline rows={3} sx={{ ...inputSx, '& .MuiInputBase-input': { fontFamily: '"Courier New", monospace' } }} />
                  </Box>
                  {selectedProjectId ? (
                    <VariableInserter projectId={selectedProjectId} onInsert={handleInsertVariableInMessage} compact={false} maxHeight={150} />
                  ) : (
                    <Box>
                      <Typography variant="subtitle2" fontWeight={600} mb={1} sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.7rem' }}>Variables globales</Typography>
                      <Box display="flex" gap={1} flexWrap="wrap">
                        {GLOBAL_VARIABLES.map(v => (<Chip key={v.key} label={`{{${v.key}}}`} size="small" onClick={() => handleInsertVariableInMessage(v.key)} sx={{ cursor: 'pointer', bgcolor: '#f5f5f5', borderRadius: 0, fontFamily: '"Courier New", monospace', fontSize: '0.7rem', '&:hover': { bgcolor: '#e0e0e0' } }} />))}
                      </Box>
                    </Box>
                  )}
                  {detectedVariables.length > 0 && (
                    <Box sx={{ p: 1.5, bgcolor: '#e3f2fd', borderRadius: 0, border: '1px solid #90caf9' }}>
                      <Typography variant="caption" sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.7rem', fontWeight: 700, color: '#1976d2', mb: 0.5, display: 'block' }}>Variables detectadas ({detectedVariables.length})</Typography>
                      <Box display="flex" gap={0.5} flexWrap="wrap">
                        {detectedVariables.map(v => (<Chip key={v} label={`{{${v}}}`} size="small" variant="outlined" sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.7rem', borderColor: '#1976d2', color: '#1976d2', borderRadius: 0 }} />))}
                      </Box>
                    </Box>
                  )}
                </>
              ...
              {formData.action === 'create_activity' && (
                <>
                  <TextField label="Título de actividad *" value={formData.actionPayload.title} onChange={(e) => handleActionPayloadChange('title', e.target.value)} fullWidth required sx={inputSx} />
                  <TextField label="Descripción" value={formData.actionPayload.description} onChange={(e) => handleActionPayloadChange('description', e.target.value)} fullWidth multiline rows={2} sx={inputSx} />
                  <FormControl size="small" fullWidth>
                    <InputLabel>Asignar a</InputLabel>
                    <Select value={formData.actionPayload.assignedTo} onChange={(e) => handleActionPayloadChange('assignedTo', e.target.value)} label="Asignar a" sx={{ ...inputSx, width: '100%' }}>
              ...
              {formData.action === 'notify_agent' && (
                <>
                  <FormControl size="small" fullWidth required>
                    ...
                    <Select value={formData.actionPayload.assignedTo} onChange={(e) => handleActionPayloadChange('assignedTo', e.target.value)} label="Notificar a *" sx={{ ...inputSx, width: '100%' }}>
                      {agents.map(agent => (<MenuItem key={agent._id} value={agent._id} sx={menuItemSx}>{agent.firstName} {agent.lastName}</MenuItem>))}
                    </Select>
                  </FormControl>
                  <TextField label="Mensaje" value={formData.actionPayload.message} onChange={(e) => handleActionPayloadChange('message', e.target.value)} fullWidth multiline rows={2} sx={inputSx} />
                </>
              )}
            </Box>
          </Box>
      ...
      <DialogActions sx={{ borderTop: '1px solid #ececec', p: 2, gap: 1, flexDirection: { xs: 'column', sm: 'row' } }}>
        {isEditing && (<>
          <Button onClick={handleTest} disabled={loading} startIcon={<PlayArrow />} sx={{ ...unifiedButtonSx, color: '#2196f3' }}>Probar</Button>
          <Button onClick={handleDelete} disabled={loading} sx={{ ...unifiedButtonSx, color: '#d32f2f' }}>Eliminar</Button>
        </>)}
        <Box sx={{ flex: 1 }} />
        <Button onClick={onClose} disabled={loading} sx={{ ...unifiedButtonSx, color: '#888' }}>Cancelar</Button>
