...
import { useResolvedProperties } from '@shared/hooks/useResolvedProperties' // Ajusta la ruta si es necesario
import { useProjects } from '@shared/hooks/useProjects'
import { useResidents } from '@shared/hooks/useResidents'
import { useSurveyColumns } from '../../../constants/Columns/surveyColumns' // Ajusta la ruta si es necesario
...
export default function SurveysPanel({ onNotify }) {
  ...
  const { stats: surveyStats, loading: statsLoading } = useSurveyStats(filters.projectId, { 
  ...
  const columns = useSurveyColumns({ 
    t, 
    propertiesMap: surveyPropertiesMap, 
    onView: handleView, 
    onEdit: handleEdit, 
  ...
  return (
    <Box>
      <Paper sx={{ borderRadius: 2, overflow: 'hidden' }}>
        ...
        >
          <Tab label={t('tabs.surveys')} />
          <Tab label={t('tabs.templates')} />
        </Tabs>
        ...
        <Box sx={{ p: 3 }}>
          {tabValue === 0 && (
            <Box>
              {!showSurveyForm && (
                <>
                  <Box sx={{ mb: 3, p: 3, bgcolor: '#f5f5f5', borderRadius: 2, border: '1px solid #e0e0e0' }}>
                      ...
                      <Grid container spacing={3}>
                        <Grid item xs={12} md={3}>
                          <Box textAlign="center">
                            <Typography variant="caption" color="text.secondary">{t('surveys.totalSurveys')}</Typography>
                            <Typography variant="h4" fontWeight={700} color="primary">{surveyStats.total}</Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={12} md={3}>
                          <Box textAlign="center">
                            <Typography variant="caption" color="text.secondary">{t('survey.rating')}</Typography>
                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mt: 1 }}>
                              <Rating value={surveyStats.avgOverallRating || 0} readOnly size="large" sx={{ color: '#ffb300' }} />
                              <Typography variant="h6" sx={{ ml: 1 }}>({(surveyStats.avgOverallRating || 0).toFixed(1)}/5)</Typography>
                            </Box>
                          </Box>
                        </Grid>
                        <Grid item xs={12} md={3}>
                          <Box textAlign="center">
                            <Typography variant="caption" color="text.secondary">{t('survey.nps')}</Typography>
                            <Typography variant="h4" fontWeight={700} color={(surveyStats.avgNps || 0) >= 9 ? 'success.main' : (surveyStats.avgNps || 0) >= 7 ? 'warning.main' : 'error.main'}>
                        ...
                        <Grid item xs={12} md={3}>
                          <Box textAlign="center">
                            <Typography variant="caption" color="text.secondary">{t('survey.templatesUsed')}</Typography>
                  ...
                  <Box sx={{ mb: 2, p: 2.5, bgcolor: '#f9f9f9', borderRadius: 2, border: '1px solid #e0e0e0' }}>
                    <Grid container spacing={2} alignItems="center">
                      ...
                      <Grid item xs={12} sm={6} md={2.4}>
                        <FormControl fullWidth size="small">
                          ...
                          <Select value={filters.templateId} onChange={(e) => handleFilterChange('templateId', e.target.value)} label={t('filters.template')}>
                            <MenuItem value="">{t('filters.all')}</MenuItem>
                  ...
                  <Box display="flex" justifyContent="flex-end" sx={{ mb: 2 }}>
                    <Button variant="contained" startIcon={<Add />} onClick={() => setShowSurveyForm(true)}>
                      {t('surveys.newSurvey')}
                    </Button>
                  </Box>
                </>
          ...
          {tabValue === 1 && (
            <TemplateManager onNotify={onNotify} />
          )}
        </Box>
      </Paper>

      <Dialog open={surveyDetailOpen} onClose={() => setSurveyDetailOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{t('surveys.surveyDetails')}</DialogTitle>
        <DialogContent dividers>
          {selectedSurvey && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Box>
                <Typography variant="caption" color="text.secondary">{t('survey.client')}</Typography>
                <Typography variant="body1" fontWeight={600}>{selectedSurvey.clientId?.firstName} {selectedSurvey.clientId?.lastName}</Typography>
              </Box>
              <Box>
                <Typography variant="caption" color="text.secondary">{t('survey.project')}</Typography>
                <Typography variant="body1" fontWeight={600}>{selectedSurvey.projectId?.name || selectedSurvey.projectId?.title?.es || t('common.na')}</Typography>
              </Box>
              <Box>
                <Typography variant="caption" color="text.secondary">{t('survey.property')}</Typography>
              ...
              <Box>
                <Typography variant="caption" color="text.secondary">{t('surveys.overallRating')}</Typography>
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  <Rating value={selectedSurvey.overallRating || 0} readOnly size="small" sx={{ color: '#ffb300' }} />
                  <Typography variant="body2" sx={{ ml: 1 }}>({selectedSurvey.overallRating || 0}/5)</Typography>
                </Box>
              </Box>
              <Box>
                <Typography variant="caption" color="text.secondary">{t('survey.responses')}</Typography>
                <Box sx={{ mt: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
                  {selectedSurvey.responses?.map((r, idx) => (
                    <Box key={idx} sx={{ p: 1.5, bgcolor: '#f9f9f9', borderRadius: 1 }}>
                      <Typography variant="body2" fontWeight={600}>{r.question || r.questionKey}</Typography>
                      <Typography variant="caption" color="text.secondary">{t('surveys.rating')}: {r.rating}/5</Typography>
                      {r.comment && <Typography variant="body2" sx={{ mt: 0.5, fontStyle: 'italic' }}>"{r.comment}"</Typography>}
                    </Box>
                  ))}
                </Box>
              </Box>
        ...
        <DialogActions>
          <Button onClick={() => setSurveyDetailOpen(false)}>{t('actions.close')}</Button>
          <Button variant="contained" startIcon={<Edit />} onClick={() => { setSurveyDetailOpen(false); handleEdit(selectedSurvey); }}>
            {t('actions.edit')}
          </Button>
        </DialogActions>
      </Dialog>

      <Dialog open={surveyDeleteDialogOpen} onClose={() => setSurveyDeleteDialogOpen(false)}>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><WarningIcon color="error" />{t('actions.confirmDelete')}</DialogTitle>
        <DialogContent><DialogContentText>{t('actions.deleteWarning')}</DialogContentText></DialogContent>
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setSurveyDeleteDialogOpen(false)}>{t('actions.cancel')}</Button>
