    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 0, border: '1px solid #ececec' } }}>
      ...
      <DialogContent dividers sx={{ bgcolor: '#fafafa', p: { xs: 2, sm: 3 } }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <TextField fullWidth required label={t('templates.name')} value={formData.name} onChange={(e) => handleChange('name', e.target.value)} placeholder={t('templates.namePlaceholder')} sx={inputSx} />

          <FormControlLabel control={<Switch checked={formData.isActive} onChange={(e) => handleChange('isActive', e.target.checked)} />} label={<Typography fontWeight={600} sx={{ fontFamily: '"Courier New", monospace' }}>{t('templates.isActive')}</Typography>} />
          ...
          {formData.questions.map((question, index) => (
            <Box key={index} sx={{ p: 2, bgcolor: 'white', borderRadius: 0, border: '1px solid #e0e0e0', position: 'relative', transition: 'all 0.2s', '&:hover': { borderColor: '#000' } }}>
      ...
      <DialogActions sx={{ p: 2, bgcolor: '#fafafa', borderTop: '1px solid #e0e0e0', flexDirection: { xs: 'column', sm: 'row' }, gap: 1 }}>
