...
const validateAppointmentDuration = (startDate, startTime, endDate, endTime, t) => {
  if (!startDate || !startTime) return { valid: false, error: t('validation.startDateRequired', 'La fecha y hora de inicio son obligatorias') }

  const startDateTime = combineDateAndTime(startDate, startTime)
  const start = new Date(startDateTime)

  if (!endDate || !endTime) return { valid: true, duration: 60 }

  const endDateTime = combineDateAndTime(endDate, endTime)
  const end = new Date(endDateTime)
  const diffMinutes = Math.round((end - start) / (1000 * 60))

  if (diffMinutes < 60) return { valid: false, error: t('duration.minError', { minutes: diffMinutes }) }
  if (diffMinutes >= 1440) return { valid: false, error: t('duration.maxError') }

  return { valid: true, duration: diffMinutes }
}

const formatDuration = (minutes, t) => {
  if (minutes < 60) return t('duration.minutes', { count: minutes, defaultValue: `${minutes} minutos` })
  const hours = Math.floor(minutes / 60)
  const mins = minutes % 60
  if (mins === 0) return t('duration.hours', { count: hours, defaultValue: `${hours} horas` })
  return t('duration.hoursMinutes', { hours, minutes: mins, defaultValue: `${hours}h ${mins}min` })
}
...
const AppointmentModal = ({
  ...
  const [formData, setFormData] = useState({
    title: '', type: 'visita', notes: '',
    startDate: '', startTime: '', endDate: '', endTime: '',
    leadId: '', clientId: '', projectId: '', assignedTo: '', status: 'pendiente'
  })
  ...
  useEffect(() => {
    if (appointment) {
      ...
      setFormData({
        title: appointment.title || '', type: appointment.type || 'visita', notes: appointment.notes || '',
        startDate: formatDateLocal(startDate), startTime: formatTimeLocal(startDate),
        endDate: formatDateLocal(endDate), endTime: formatTimeLocal(endDate),
        leadId, clientId, projectId, assignedTo: assignedToId, status: appointment.status || 'pendiente'
      })
      setContactType(clientId ? 1 : 0)
    } else if (prefillData) {
      const prefillDate = prefillData.startDate ? new Date(prefillData.startDate) : prefillData.date ? new Date(prefillData.date) : null
      
      setFormData(prev => ({
        ...prev,
        leadId: prefillData.leadId || '', clientId: prefillData.clientId || '',
        projectId: prefillData.projectId || '', assignedTo: prefillData.assignedTo || '',
        ...
        endTime: prefillDate ? formatTimeLocal(new Date(prefillDate.getTime() + 60 * 60000)) : ''
      }))
      setContactType(prefillData.clientId ? 1 : 0)
    } else {
      setFormData({ title: '', type: 'visita', notes: '', startDate: '', startTime: '', endDate: '', endTime: '', leadId: '', clientId: '', projectId: '', assignedTo: '', status: 'pendiente' })
      setContactType(0)
    }
    setError(null)
  ...
  useEffect(() => {
    if (formData.startDate && formData.startTime) {
      const validation = validateAppointmentDuration(formData.startDate, formData.startTime, formData.endDate, formData.endTime, t)
      if (validation.valid) {
        setDurationInfo({ valid: true, duration: validation.duration, formatted: formatDuration(validation.duration, t) })
      } else {
        setDurationInfo({ valid: false, error: validation.error })
      }
    } else {
      setDurationInfo(null)
    }
  ...
  const handleContactTypeChange = (event, newValue) => {
    ...
    if (newValue === 0) setFormData(prev => ({ ...prev, clientId: '' }))
    else setFormData(prev => ({ ...prev, leadId: '' }))
  }

  const handleSubmit = async () => {
    if (!formData.title) return setError(t('validation.titleRequired'))
    if (!formData.type) return setError(t('validation.typeRequired'))
    if (!formData.startDate || !formData.startTime) return setError(t('validation.startDateRequired'))
    if (!formData.projectId) return setError(t('validation.projectRequired'))
    if (!formData.assignedTo) return setError(t('validation.assignedToRequired'))
    if (!formData.leadId && !formData.clientId) return setError(t('validation.contactRequired'))

    const durationValidation = validateAppointmentDuration(formData.startDate, formData.startTime, formData.endDate, formData.endTime, t)
    if (!durationValidation.valid) return setError(durationValidation.error)
    ...
    try {
      ...
      const payload = {
        title: formData.title, type: formData.type, notes: formData.notes,
        startDate: startDateISO, endDate: endDateISO,
        projectId: formData.projectId, assignedTo: formData.assignedTo, status: formData.status
      }
      
      if (contactType === 0 && formData.leadId) payload.leadId = formData.leadId
      else if (contactType === 1 && formData.clientId) payload.clientId = formData.clientId

      if (isEditing) await onSave?.(appointment._id, payload)
  ...
  // ✅ Estilos unificados para Dropdowns
  const menuItemSx = {
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 0, border: '1px solid #ececec' } }}>
      ...
      <DialogTitle id="appointment-modal-title" sx={{ borderBottom: '1px solid #ececec', display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: { xs: 2, sm: 3 } }}>
        <Box display="flex" alignItems="center" gap={1}>
          ...
          {isEditing && (
            <Box sx={{ ml: 1, px: 1, py: 0.3, bgcolor: currentStatusConfig.color, color: '#fff', fontFamily: '"Courier New", monospace', fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.5px', textTransform: 'uppercase', borderRadius: 0 }}>
        ...
        <Box display="flex" flexDirection="column" gap={2.5} sx={{ p: 3 }}>
          ...
          <Box id="appointment-modal-type-status" display="flex" flexDirection={{ xs: 'column', sm: 'row' }} gap={2}>
            ...
            <FormControl size="small" fullWidth required>
              ...
              <Select value={formData.status} onChange={(e) => handleChange('status', e.target.value)} label={`${t('form.status')} *`} sx={{ ...inputSx, width: '100%' }}>
                {APPOINTMENT_STATUSES.map(status => (
                  <MenuItem key={status.value} value={status.value} sx={menuItemSx}>
                    <Box display="flex" alignItems="center" gap={1}>
          ...
          <Box id="appointment-modal-contact">
            ...
            <Tabs value={contactType} onChange={handleContactTypeChange} sx={{ borderBottom: '1px solid #ececec', mb: 2, '& .MuiTab-root': { fontFamily: '"Courier New", monospace', fontSize: '0.75rem', letterSpacing: '0.5px', textTransform: 'none', minHeight: 40, borderRadius: 0 } }}>
            ...
            {contactType === 0 ? (
              <FormControl size="small" fullWidth required>
                <InputLabel>{`${t('contact.lead')} *`}</InputLabel>
                <Select value={formData.leadId} onChange={(e) => handleChange('leadId', e.target.value)} label={`${t('contact.lead')} *`} sx={{ ...inputSx, width: '100%' }}>
                  <MenuItem value="" sx={menuItemSx}><em>{t('form.selectLead')}</em></MenuItem>
                  {leads.map(lead => (
                    <MenuItem key={lead._id} value={lead._id} sx={menuItemSx}>
              ...
              <FormControl size="small" fullWidth required>
                <InputLabel>{`${t('contact.client')} *`}</InputLabel>
                <Select value={formData.clientId} onChange={(e) => handleChange('clientId', e.target.value)} label={`${t('contact.client')} *`} sx={{ ...inputSx, width: '100%' }}>
                  <MenuItem value="" sx={menuItemSx}><em>{t('form.selectClient')}</em></MenuItem>
          ...
          <FormControl size="small" fullWidth required>
            ...
            <Select value={formData.assignedTo} onChange={(e) => handleChange('assignedTo', e.target.value)} label={`${t('form.assignedTo')} *`} sx={{ ...inputSx, width: '100%' }}>
              {agents.map(agent => (
                <MenuItem key={agent._id} value={agent._id} sx={menuItemSx}>
          ...
          <Box id="appointment-modal-datetime">
            ...
            <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} gap={2}>
              <TextField label={t('form.endDate')} type="date" value={formData.endDate} onChange={(e) => handleChange('endDate', e.target.value)} InputLabelProps={{ shrink: true }} fullWidth sx={inputSx} />
              <TextField label={t('form.endTime')} type="time" value={formData.endTime} onChange={(e) => handleChange('endTime', e.target.value)} InputLabelProps={{ shrink: true }} fullWidth sx={inputSx} />
            </Box>

            {durationInfo && (
              <Box sx={{ mt: 1.5, p: 1.5, bgcolor: durationInfo.valid ? '#e8f5e9' : '#ffebee', border: `1px solid ${durationInfo.valid ? '#4caf50' : '#f44336'}`, borderRadius: 0, display: 'flex', alignItems: 'center', gap: 1 }}>
                <AccessTime sx={{ fontSize: 16, color: durationInfo.valid ? '#2e7d32' : '#c62828' }} />
                <Typography variant="caption" sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.75rem', fontWeight: 600, color: durationInfo.valid ? '#2e7d32' : '#c62828' }}>
          ...
          <TextField label={t('form.notes')} value={formData.notes} onChange={(e) => handleChange('notes', e.target.value)} fullWidth multiline rows={3} sx={{ ...inputSx, '& .MuiInputBase-input': { fontFamily: '"Helvetica Neue", sans-serif' } }} />
        </Box>
      </DialogContent>
      ...
      <DialogActions id="appointment-modal-actions" sx={{ borderTop: '1px solid #ececec', p: 2, gap: 1, flexDirection: { xs: 'column', sm: 'row' } }}>
        ...
        <Button onClick={onClose} disabled={loading} sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.75rem', color: '#888', textTransform: 'none', letterSpacing: '0.5px', borderRadius: 0, width: { xs: '100%', sm: 'auto' }, '&:hover': { bgcolor: '#f5f5f5' } }}>
          {t('form.cancel')}
        </Button>
        <Button onClick={handleSubmit} variant="contained" startIcon={loading ? <CircularProgress size={16} /> : <Event />} disabled={loading || (durationInfo && !durationInfo.valid)} sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.75rem', textTransform: 'none', letterSpacing: '0.5px', bgcolor: '#000', borderRadius: 0, width: { xs: '100%', sm: 'auto' }, '&:hover': { bgcolor: '#222', boxShadow: '6px 6px 0px rgba(0,0,0,0.12)' } }}>
