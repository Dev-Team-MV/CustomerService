...
const initialFormData = {
  title: '', description: '', columnId: '', priority: 'medium', dueDate: null,
  assignedTo: null, tags: [], relatedProjects: [], contactType: 'none',
  contact: null, externalContact: { name: '', phone: '', email: '' }
}

const ActivityModal = ({ 
  open, onClose, activity = null, columns = [],
  ...
  const adminUserOptions = users
    ...
    .map(u => ({ _id: u._id, name: `${u.firstName || ''} ${u.lastName || ''}`.trim() || u.email, email: u.email, phone: u.phoneNumber || '', role: u.role }))

  const allUserOptions = users.map(u => ({ _id: u._id, name: `${u.firstName || ''} ${u.lastName || ''}`.trim() || u.email, email: u.email, phone: u.phoneNumber || '' }))
  const projectOptions = projects.map(p => ({ _id: p._id, name: p.name || p.title?.es || p.title?.en || p.slug || 'Sin nombre', slug: p.slug, status: p.status }))

  useEffect(() => {
    if (activity) {
      const assignee = activity.assignedTo
      const relatedProjs = (activity.relatedProjects || []).map(p => {
        if (typeof p === 'object') return { _id: p._id, name: p.name || p.title?.es || p.title?.en || p.slug || 'Sin nombre', slug: p.slug, status: p.status }
        const found = projects.find(proj => proj._id === p)
        return found ? { _id: found._id, name: found.name || found.title?.es || found.title?.en || found.slug || 'Sin nombre', slug: found.slug, status: found.status } : null
      }).filter(Boolean)

      let contactType = 'none', contact = null, externalContact = { name: '', phone: '', email: '' }
      if (activity.contact) {
        if (activity.contact.user) {
          contactType = 'registered'
          const contactUser = users.find(u => u._id === activity.contact.user)
          contact = contactUser ? { _id: contactUser._id, name: `${contactUser.firstName || ''} ${contactUser.lastName || ''}`.trim() || contactUser.email, email: contactUser.email, phone: contactUser.phoneNumber || '' } : { _id: activity.contact.user, name: activity.contact.name || '', email: activity.contact.email || '', phone: activity.contact.phone || '' }
        } else if (activity.contact.name) {
          contactType = 'external'
          externalContact = { name: activity.contact.name || '', phone: activity.contact.phone || '', email: activity.contact.email || '' }
        }
      }

      setFormData({
        title: activity.title || '', description: activity.description || '',
        columnId: typeof activity.columnId === 'object' ? activity.columnId._id : (activity.columnId || columns[0]?._id || ''),
        priority: activity.priority || 'medium', dueDate: activity.dueDate ? new Date(activity.dueDate) : null,
        assignedTo: assignee ? { _id: assignee._id, name: `${assignee.firstName || ''} ${assignee.lastName || ''}`.trim(), email: assignee.email, phone: assignee.phoneNumber || '' } : null,
        tags: activity.tags || [], relatedProjects: relatedProjs, contactType, contact, externalContact
      })
    } else {
      setFormData({ ...initialFormData, columnId: columns[0]?._id || '' })
    }
  }, [activity, open, columns, projects, users])

  const handleChange = (field, value) => setFormData(prev => ({ ...prev, [field]: value }))
  const handleExternalContactChange = (field, value) => setFormData(prev => ({ ...prev, externalContact: { ...prev.externalContact, [field]: value } }))
  const handleContactTypeChange = (type) => setFormData(prev => ({ ...prev, contactType: type, contact: null, externalContact: { name: '', phone: '', email: '' } }))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.contactType === 'registered' && formData.contact) {
        contactPayload = { user: formData.contact._id, name: formData.contact.name, phone: formData.contact.phone || '', email: formData.contact.email || '' }
      } else if (formData.contactType === 'external' && formData.externalContact.name) {
        contactPayload = { name: formData.externalContact.name, phone: formData.externalContact.phone || '', email: formData.externalContact.email || '' }
      }

      const payload = {
        title: formData.title, description: formData.description, columnId: formData.columnId,
        priority: formData.priority, dueDate: formData.dueDate || undefined,
        assignedTo: formData.assignedTo?._id || undefined, tags: formData.tags,
        relatedProjects: formData.relatedProjects.map(p => p._id), contact: contactPayload
      }
            ...
            <Box flex={{ xs: 'none', lg: '1 1 60%' }} width={{ xs: '100%', lg: 'auto' }} display="flex" flexDirection="column" gap={2.5} py={1}>
              ...
              <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} gap={2}>
                <FormControl fullWidth>
                  ...
                  <Select value={formData.columnId} label={t('activities.form.column')} onChange={(e) => handleChange('columnId', e.target.value)} sx={inputSx}>
                    {columns.map(col => (
                      <MenuItem key={col._id} value={col._id} sx={{ fontFamily: '"Courier New", monospace' }}>
                        <Box display="flex" alignItems="center" gap={1}>
                          <Box sx={{ width: 8, height: 8, borderRadius: 0, bgcolor: col.color }} />
                          {col.name}
                        </Box>
                      </MenuItem>
                    ))}
                ...
                <FormControl fullWidth>
                  ...
                  <Select value={formData.priority} label={t('activities.form.priority')} onChange={(e) => handleChange('priority', e.target.value)} sx={inputSx}>
                    {ACTIVITY_PRIORITIES.map(p => (
                      <MenuItem key={p.id} value={p.id} sx={{ fontFamily: '"Courier New", monospace' }}>
                        <Chip label={t(`activities.priority.${p.id}`)} size="small" sx={{ bgcolor: `${p.color}20`, color: p.color, height: 22, borderRadius: 0, fontFamily: '"Courier New", monospace', fontSize: '0.65rem' }} />
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
              ...
              <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} gap={2}>
                ...
                <Autocomplete
                  ...
                  fullWidth
                  renderInput={(params) => <TextField {...params} label={t('activities.form.assignedTo')} placeholder="Buscar administrador..." sx={inputSx} />}
                  renderOption={(props, option) => (
                    <Box component="li" {...props} key={option._id} display="flex" alignItems="center" gap={1.5} sx={{ borderRadius: 0 }}>
                      <Avatar sx={{ width: 32, height: 32, bgcolor: option.role === 'superadmin' ? '#9c27b0' : '#2196f3', borderRadius: 0 }}>
                        {option.name?.charAt(0)}
                      </Avatar>
                      <Box>
                        <Typography variant="body2" fontWeight={500} sx={{ fontFamily: '"Helvetica Neue", sans-serif' }}>{option.name}</Typography>
                        <Typography variant="caption" color="text.secondary" sx={{ fontFamily: '"Courier New", monospace' }}>{option.email} • {option.role}</Typography>
                      </Box>
                    </Box>
                  )}
                />
              ...
              <Box>
                ...
                {formData.contactType === 'external' && (
                  <Paper variant="outlined" sx={{ p: 2, borderRadius: 0, border: '1px solid #e0e0e0', bgcolor: '#fff8e1' }}>
                    <Box display="flex" alignItems="center" gap={1} mb={2}>
                      <PersonAdd color="warning" />
                      <Typography variant="body2" color="warning.main" fontWeight={500} sx={{ fontFamily: '"Courier New", monospace' }}>{t('activities.contact.unregisteredContact')}</Typography>
                    </Box>
              ...
              <Box>
                <Typography variant="subtitle2" mb={1} sx={{ fontFamily: '"Courier New", monospace', fontSize: '0.7rem', letterSpacing: '1px', textTransform: 'uppercase' }}>{t('activities.form.relatedProjects')}</Typography>
                <Autocomplete
                  multiple
