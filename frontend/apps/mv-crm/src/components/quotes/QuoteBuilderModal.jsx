...
export default function QuoteBuilderModal({ open, onClose, quote = null, onSave, projects = [], leads = [], clients = [] }) {
  ...
  const [formData, setFormData] = useState({
    ...
    hasModelUpgrade: false,
    hasModelBalcony: false,
  ...
  useEffect(() => {
    if (quote) {
      setFormData({
        ...
        // ✅ MAPEO INVERSO: Leer del formato del backend al formato del formulario
  ...
  useEffect(() => {
    if (formData.apartmentId) {
      const selectedApt = apartments.find(a => a._id === formData.apartmentId)
      if (selectedApt) {
        setFormData(prev => ({ ...prev, selectedRenderType: selectedApt.selectedRenderType || 'basic' }))
      }
    }
  }, [formData.apartmentId, apartments])

  // 9. CÁLCULO AUTOMÁTICO DEL PRECIO TOTAL
  useEffect(() => {
    let newTotalPrice = 0
    ...
    if (projectType === 'lakewood' || projectType === 'property') {
      if (formData.lotId) {
        ...
        if (modelPricingOptions?.availableOptions) {
          const available = modelPricingOptions.availableOptions
          if (formData.hasModelUpgrade && available.upgrades?.[0]) newTotalPrice += available.upgrades[0].price
          if (formData.hasModelBalcony && available.balconies?.[0]) newTotalPrice += available.balconies[0].price
          if (formData.hasModelStorage && available.storages?.[0]) newTotalPrice += available.storages[0].price
        }
      }
    } else if (projectType === '6town' && formData.buildingId) {
      const selectedBuilding = buildings.find(b => b._id === formData.buildingId)
    ...
    formData.lotId, formData.modelId, formData.facadeId, formData.deckId, 
    formData.apartmentId, formData.buildingId, formData.selectedOptions, 
    formData.selectedRenderType, formData.hasModelUpgrade, formData.hasModelBalcony, formData.hasModelStorage,
    lots, models, facades, apartments, buildings, projectType, catalogConfig, modelPricingOptions
  ])
  ...
  useEffect(() => {
    if (formData.totalPrice > 0 && formData.termMonths > 0 && activeStep >= 1) {
      ...
      for (let i = 1; i <= formData.termMonths; i++) {
        ...
        schedule.push({ 
          monthNumber: i, date: date.toISOString(), 
          principal: Math.round(principal * 100) / 100, 
          interest: Math.round(interest * 100) / 100, 
          payment: Math.round(payment * 100) / 100, 
          balance: Math.round(balance * 100) / 100, 
  ...
  // ✅ CORREGIDO: Transformación del payload para que coincida con lo que espera el backend
  const handleSubmit = async () => {
    setLoading(true)
    setError(null)
    try {
      ...
      // ✅ MAPEO ESPECÍFICO PARA LA CREACIÓN DE PROPIEDAD (LakeWood)
      if (projectType === 'lakewood' || projectType === 'property') {
        payload = {
          ...payload,
                        ...
                        <Box display="flex" flexDirection="column" gap={1.5}>
                          {modelPricingOptions.availableOptions.upgrades?.length > 0 && (
                            <FormControlLabel
                              control={
                                <Switch
                                  checked={formData.hasModelUpgrade}
                                  onChange={() => handleModelCustomizationToggle('upgrade')}
                                  color="primary"
                                />
                              }
                              label={
                                <Box display="flex" alignItems="center" gap={1}>
                                  <Typography variant="body2">
                                    {modelPricingOptions.availableOptions.upgrades[0].name || t('form.upgrade', 'Upgrade')}
                                  </Typography>
                                  <Chip 
                                    label={`+$${modelPricingOptions.availableOptions.upgrades[0].price?.toLocaleString()}`} 
                                    size="small" 
                                    color="primary" 
                                    variant="outlined" 
                          ...
                          {modelPricingOptions.availableOptions.balconies?.length > 0 && (
                            <FormControlLabel
                              control={
                                <Switch
                                  checked={formData.hasModelBalcony}
                                  onChange={() => handleModelCustomizationToggle('balcony')}
                                  color="primary"
                                />
                              }
                              label={
                                <Box display="flex" alignItems="center" gap={1}>
                                  <Typography variant="body2">
                                    {modelPricingOptions.availableOptions.balconies[0].name || t('form.balcony', 'Balcón')}
                                  </Typography>
                                  <Chip 
                                    label={`+$${modelPricingOptions.availableOptions.balconies[0].price?.toLocaleString()}`} 
                                    size="small" 
                                    color="primary" 
                                    variant="outlined" 
                                    sx={{ height: 20, fontSize: '0.7rem' }} 
                          ...
                          {modelPricingOptions.availableOptions.storages?.length > 0 && (
                            <FormControlLabel
                              control={
                                <Switch
                                  checked={formData.hasModelStorage}
                                  onChange={() => handleModelCustomizationToggle('storage')}
                                  color="primary"
                                />
                              }
                              label={
                                <Box display="flex" alignItems="center" gap={1}>
                                  <Typography variant="body2">
                                    {modelPricingOptions.availableOptions.storages[0].name || t('form.storage', 'Storage')}
                                  </Typography>
                                  <Chip 
                                    label={`+$${modelPricingOptions.availableOptions.storages[0].price?.toLocaleString()}`} 
                                    size="small" 
                                    color="primary" 
                                    variant="outlined" 
                                    sx={{ height: 20, fontSize: '0.7rem' }} 
