...
const LeadDetails = ({ lead, open, onClose, onEdit, onDelete, onConvert, stages = [] }) => {
      ...
      <Box sx={{ p: 2, overflowY: 'auto', flex: 1 }}>
        {isLost && lead.lostReason && (
          <Alert severity="error" icon={<Warning />} sx={{ mb: 3 }}>
            <Typography variant="subtitle2" fontWeight={600} gutterBottom>{t('lostReason') || 'Razón de pérdida'}</Typography>
            <Typography variant="body2">{lead.lostReason}</Typography>
          </Alert>
        )}
            ...
            <Box display="flex" flexDirection="column" gap={1.5}>
              {lead.email && (
                <Box display="flex" alignItems="center" gap={1.5}>
                  <Avatar sx={{ bgcolor: '#2196f3', width: 32, height: 32 }}><Email sx={{ fontSize: 18 }} /></Avatar>
                  <Box>
              ...
              {lead.phone && (
                <Box display="flex" alignItems="center" gap={1.5}>
                  <Avatar sx={{ bgcolor: '#4caf50', width: 32, height: 32 }}><Phone sx={{ fontSize: 18 }} /></Avatar>
                  <Box>
        ...
        {project && (
          <Box mb={3}>
            <Typography variant="subtitle2" fontWeight={600} mb={1.5}>{t('form.project')}</Typography>
            <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: '#e3f2fd' }}>
              <Box display="flex" alignItems="center" gap={1.5}>
                <Avatar sx={{ bgcolor: '#1976d2', width: 32, height: 32 }}><Business sx={{ fontSize: 18 }} /></Avatar>
                <Typography fontWeight={600} sx={{ color: '#1565c0' }}>{project.name || project.title?.es || project.title?.en || 'Sin nombre'}</Typography>
              </Box>
            </Paper>
          </Box>
        )}
        ...
        {assignee && (
          <Box mb={3}>
            <Typography variant="subtitle2" fontWeight={600} mb={1.5}>{t('form.assignedTo')}</Typography>
            <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
              <Box display="flex" alignItems="center" gap={1.5}>
                <Avatar sx={{ bgcolor: '#9c27b0', width: 32, height: 32 }}>{assignee.firstName?.charAt(0) || '?'}</Avatar>
                <Box>
                  <Typography fontWeight={600}>{`${assignee.firstName || ''} ${assignee.lastName || ''}`.trim()}</Typography>
                  {assignee.email && <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>{assignee.email}</Typography>}
                </Box>
              </Box>
            </Paper>
          </Box>
        ...
        {lead.notes && (
          <Box mb={3}>
            <Typography variant="subtitle2" fontWeight={600} mb={1}>{t('form.notes')}</Typography>
            <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: '#fafafa' }}>
              <Typography variant="body2" color="text.secondary" sx={{ whiteSpace: 'pre-wrap' }}>{lead.notes}</Typography>
            </Paper>
          </Box>
        )}
        ...
        <Box display="flex" alignItems="center" gap={1} mb={1}>
          ...
          <Typography variant="caption" color="text.secondary">{t('details.created')}: {formatDate(lead.createdAt, locale)}</Typography>
        </Box>
        {lead.updatedAt && lead.updatedAt !== lead.createdAt && (
          <Box display="flex" alignItems="center" gap={1} mb={1}>
            <CalendarToday sx={{ fontSize: 16, color: '#757575' }} />
            <Typography variant="caption" color="text.secondary">{t('details.updated') || 'Actualizado'}: {formatDate(lead.updatedAt, locale)}</Typography>
          </Box>
        )}
      </Box>
