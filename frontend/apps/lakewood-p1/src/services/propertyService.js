...
export const propertyService = {
    ...
    downloadBulkCombinedStatementPdf: async (propertyIds, suggestedFilename = 'estados-combinados.pdf') => {
        const token = localStorage.getItem('token')
        const baseURL = import.meta.env.VITE_API_URL || 'http://localhost:5001/api'
        const response = await fetch(`${baseURL}/properties/bulk/combined-statement/pdf`, {
            method: 'POST',
            headers: {
    ...
    downloadBalanceGeneralPdf: async (propertyIds, suggestedFilename = 'balance-general.pdf') => {
        const token = localStorage.getItem('token')
        const baseURL = import.meta.env.VITE_API_URL || 'http://localhost:5001/api'
        const response = await fetch(`${baseURL}/properties/bulk/balance-general/pdf`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...(token ? { Authorization: `Bearer ${token}` } : {})
