...
const inputSx = {
  ...
  '& .MuiInputLabel-root': {
    fontFamily: '"DM Sans", sans-serif', fontWeight: 500, color: '#706f6f',
...
const outlinedBtnSx = {
  ...
  borderColor: 'rgba(140, 165, 81, 0.3)', borderWidth: '2px',
  color: '#333F1F', fontFamily: '"DM Sans", sans-serif', fontWeight: 600,
        ...
        <>
          <Button
            ...
            sx={{
              ...
              fontFamily: '"DM Sans", sans-serif', border: '2px solid #e0e0e0',
      ...
      <Grid container spacing={3}>
          ...
          <Box mb={3}>
            ...
            <Button
              ...
              sx={{
                ...
                borderWidth: 2, py: 2, color: '#706f6f',
            ...
            {formData.heroImage && (
              <Box sx={{ position: 'relative', mt: 2 }}>
                <Box
                  component="img" src={formData.heroImage}
                  sx={{ width: '100%', height: 200, objectFit: 'cover', borderRadius: 3, border: '1px solid #e0e0e0' }}
                />
                <IconButton
                  size="small" onClick={() => setField('heroImage', null)}
                  sx={{ position: 'absolute', top: 8, right: 8, bgcolor: 'rgba(229, 134, 60, 0.9)', color: 'white', '&:hover': { bgcolor: '#E5863C', transform: 'scale(1.1)' } }}
                >
          ...
          <Box>
            ...
            <Box display="flex" gap={1} flexWrap="wrap" mb={3} p={2} sx={{ bgcolor: '#fafafa', borderRadius: 3, border: '1px solid #e0e0e0' }}>
              {BLOCK_TYPES.map(({ type, icon, label }) => (
                <Button
                  key={type} size="small" startIcon={icon}
                  onClick={() => addBlock(type)}
                  sx={{ textTransform: 'none', borderRadius: 2, fontFamily: '"DM Sans", sans-serif', fontWeight: 600, color: '#333F1F', '&:hover': { bgcolor: 'rgba(140, 165, 81, 0.08)' } }}
                >
                  {label}
                </Button>
              ))}
                ...
                {(provided, snapshot) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 12, transition: 'background-color 0.2s',
                      minHeight: contentBlocks.length === 0 ? 100 : 'auto', padding: 4
                    }}
                  >
                    {contentBlocks.map((block, index) => (
                      <Draggable key={block.id} draggableId={block.id} index={index}>
                        {(provided, snapshot) => (
                          <div ref={provided.innerRef} {...provided.draggableProps} style={{ marginBottom: 16, ...provided.draggableProps.style }}>
                            <Paper
                              elevation={snapshot.isDragging ? 8 : 0}
                              sx={{
                                p: 2, borderRadius: 3, transition: 'all 0.2s',
                              ...
                              <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                                <Box display="flex" alignItems="center" gap={1}>
                                  ...
                                  <Chip
                                    ...
                                    size="small"
                                    sx={{ bgcolor: 'rgba(140, 165, 81, 0.12)', color: '#8CA551', border: '1px solid rgba(140, 165, 81, 0.3)', fontWeight: 600, textTransform: 'capitalize', fontFamily: '"DM Sans", sans-serif' }}
                                  />
                                </Box>
                                <IconButton size="small" onClick={() => deleteBlock(block.id)} sx={{ color: '#E5863C', '&:hover': { bgcolor: 'rgba(229, 134, 60, 0.08)' } }}>
                                  <Delete fontSize="small" />
                                </IconButton>
                              </Box>
                              <NewsBlockEditor
        ...
        <Grid item xs={12} md={4}>
          <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid #e0e0e0', position: 'sticky', top: 20, bgcolor: '#fafafa' }}>
            ...
            {/* Category */}
            <TextField
              fullWidth select label={t('news:category')}
            ...
            {/* Status */}
            <TextField
              fullWidth select label={t('news:status')}
            ...
            <Box mb={3}>
              ...
              <Box display="flex" gap={1} flexWrap="wrap">
                {formData.tags.map((tag, idx) => (
                  <Chip
                    key={idx} label={tag} size="small" onDelete={() => deleteTag(tag)}
                    sx={{ bgcolor: 'rgba(140, 165, 81, 0.12)', color: '#8CA551', border: '1px solid rgba(140, 165, 81, 0.3)', fontFamily: '"DM Sans", sans-serif', fontWeight: 600 }}
                  />
                ))}
              </Box>
            </Box>
            ...
            <Box mb={3}>
              ...
              <Button
                ...
                disabled={uploadingImages} sx={outlinedBtnSx}
              >
                  ...
                  {images.map((img, idx) => (
                    <Box key={idx} sx={{ position: 'relative' }}>
                      ...
                      <IconButton
                        ...
                        sx={{ position: 'absolute', top: -8, right: -8, bgcolor: 'rgba(229, 134, 60, 0.9)', color: 'white', width: 20, height: 20, '&:hover': { bgcolor: '#E5863C', transform: 'scale(1.1)' } }}
                      >
            ...
            <Box>
              ...
              <Button
                ...
                disabled={uploadingVideos} sx={outlinedBtnSx}
              >
                ...
                <Box mt={2}>
                  {videos.map((_, idx) => (
                    <Chip
                      ...
                      size="small"
                      sx={{ mr: 1, mb: 1, bgcolor: 'rgba(229, 134, 60, 0.12)', color: '#E5863C', border: '1px solid rgba(229, 134, 60, 0.3)', fontFamily: '"DM Sans", sans-serif', fontWeight: 600 }}
                    />
                  ))}
                </Box>
              )}
