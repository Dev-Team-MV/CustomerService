...
import { getModelImagesForCombination } from "../services/modelImageService";
...
const Models = () => {
  ...
  const getGalleryCategories = (model) => {
    ...
    if (hasBalcony) {
      categories.push({
        ...
        interiorImages: model.balconies[0].images?.interior || [], // Interior base
      });
    }
  ...
  const handleViewPricing = async (model) => {
    try {
      const response = await api.get(`/models/${model._id}`);
  ...
  const handleOpenFacadeDialog = async (model, facade = null) => {
    ...
    if (facade) {
      ...
      const urls = Array.isArray(facade.url)
        ? facade.url
        : facade.url
          ? [facade.url]
          : [];
  ...
  // NUEVO: Handler para subir imágenes de modelo (exterior/interior, base/balcony/upgrade/storage)
  const handleFileImageUpload = async (e) => {
    ...
    try {
      ...
      const section = currentImageSection;
      const type = currentImageType;
      // Si es interior y tiene roomType, guardar como objeto
      const imagesData =
        type === "interior" && currentRoomType !== "general"
  ...
  // NUEVO: Handler para subir imágenes de fachada (puede ser múltiple)
  ...
  // Upload handler para imágenes de Deck
 const handleFileDeckUpload = async (e) => {
   ...
   try {
     const urls = [];
     for (const file of files) {
       // intenta uploadDeckImage, si no existe usa uploadModelImage como fallback
       const uploader =
            ...
            <>
              ...
              >
                <Tabs
                  ...
                  sx={{ px: 2 }}
                >
                  {getGalleryCategories(selectedModelForGallery).map(
                    (category, index) => (
                      <Tab
                        key={category.key}
                  ...
                  <>
                    ...
                    >
                      <Typography variant="body2" fontWeight="600">
                        {getCurrentImageLabel()} • {galleryImageIndex + 1} /{" "}
                  ...
                  <Grid item xs={12} sm={4}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          balconyPrice: Number(e.target.value),
                        })
                      }
                      required
                      InputProps={{
                      ...
                      <Checkbox
                        ...
                        onChange={(e) => {
                          setFormData({
                            ...
                            upgradePrice: e.target.checked
                              ? formData.upgradePrice
                              : 0,
                          });
                          if (e.target.checked) {
                            setExpandedAccordions((prev) => ({
                  ...
                  <Grid item xs={12} sm={4}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          upgradePrice: Number(e.target.value),
                        })
                      }
                      required
                      InputProps={{
                  ...
                  <Grid item xs={12} sm={4}>
                    <TextField
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          storagePrice: Number(e.target.value),
                        })
                      }
                      required
                      InputProps={{
              ...
              {pricingOptions.pricingByType.upgrade.length > 0 && (
                <Accordion defaultExpanded sx={{ mt: 2 }}>
                  <AccordionSummary expandIcon={<ExpandMore />}>
                    <Typography variant="h6">
                      Upgrade Options (
                      {pricingOptions.pricingByType.upgrade.length})
                    </Typography>
                  </AccordionSummary>
                  <AccordionDetails>
                    <Grid container spacing={2}>
                      {pricingOptions.pricingByType.upgrade.map(
                        (option, index) => (
                          <Grid item xs={12} sm={6} key={index}>
                            <Card
                              variant="outlined"
