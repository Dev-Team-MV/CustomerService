...
// import { getModelImagesForCombination, getGalleryCategories } from '../services/modelImageService'
...
//   // build gallery from modelImageService, respecting user customizations
//   const modelGallery = (() => {
//     if (!propertyDetails?.model) return { exterior: [], interior: [], combined: [], categories: [] }
  
//     const options = {
//       // ajusta keys según tu API (customizations / selectedOptions)
//       balcony: Boolean(propertyDetails.customizations?.balcony || propertyDetails.selectedOptions?.balcony),
//       upgrade: Boolean(propertyDetails.customizations?.upgrade || propertyDetails.selectedOptions?.upgrade),
//       storage: Boolean(propertyDetails.customizations?.storage || propertyDetails.selectedOptions?.storage)
//     }
  
//     const images = getModelImagesForCombination(propertyDetails.model, options) || {}
//     const exterior = Array.isArray(images.exterior) ? images.exterior : []
//     const interior = Array.isArray(images.interior) ? images.interior : []
  
//     // combinar y dedupe
//     const combined = [...new Set([...(exterior || []), ...(interior || [])])].slice(0, 20)
  
//     // normalizar categories a array de strings (si service devuelve objetos)
//     let rawCategories = []
//     try {
//       rawCategories = typeof getGalleryCategories === 'function' ? getGalleryCategories(propertyDetails.model) : []
//     } catch (e) {
//       rawCategories = []
//     }
...
//   // Dentro del componente MyProperty
// const [currentPhaseIndex, setCurrentPhaseIndex] = useState(0)

// // Cuando cambian las fases, asegúrate de que el slider esté en la primera fase incompleta
// useEffect(() => {
//   if (phases.length > 0) {
...
//   {/* Gallery: preview principal + miniaturas, categorias */}
//   <Box sx={{ mb: 3 }}>
//     <Box display="flex" gap={2} alignItems="flex-start" flexDirection={{ xs: 'column', md: 'row' }}>
//       <Box sx={{ flex: 1, bgcolor: '#f5f7fa', borderRadius: 2, p: 1, minHeight: 240, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
//         {modelGallery.combined[0] ? (
//           <img
//             src={modelGallery.combined[0]}
//             alt="Model preview"
//             style={{ maxWidth: '100%', maxHeight: 420, objectFit: 'cover', borderRadius: 8, cursor: 'pointer' }}
//             onClick={() => setSelectedImage(modelGallery.combined[0])}
//           />
//         ) : (
//           <Typography variant="caption" color="text.secondary">No model images available</Typography>
//         )}
//       </Box>

//       <Box sx={{ width: { xs: '100%', md: 240 }, mt: { xs: 1, md: 0 } }}>
//         <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 1 }}>
//           <Chip label="All" size="small" />
//           <Chip label={`Exterior (${modelGallery.exterior.length})`} size="small" />
//           <Chip label={`Interior (${modelGallery.interior.length})`} size="small" />
//           {modelGallery.categories.slice(0, 4).map((c, i) => <Chip key={i} label={c} size="small" />)}
//         </Box>

//         <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1 }}>
//           {modelGallery.combined.slice(0, 9).map((url, i) => (
//             <Box
//               key={i}
//               sx={{ width: '100%', height: 64, borderRadius: 1, overflow: 'hidden', cursor: 'pointer', border: '1px solid rgba(0,0,0,0.06)' }}
...
//         {modelGallery.combined.length > 9 && (
//           <Typography variant="caption" color="text.secondary" sx={{ mt: 1 }}>
//             +{modelGallery.combined.length - 9} more
//           </Typography>
//         )}
//       </Box>
//     </Box>
