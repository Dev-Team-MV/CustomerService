...
const ViewModels = () => {
    const { t } = useTranslation(['models', 'common']) // <-- Agrega esto
  ...
  if (loading) return <Box p={6} textAlign="center"><Typography>{t('models:loading', 'Loading models...')}</Typography></Box>

  return (
    <Container maxWidth="xl" sx={{ p: 4 }}>
      <PageHeader
        icon={CompareArrows}
        title={t('models:viewAllTitle', 'All Models')}
        subtitle={t('models:viewAllSubtitle', 'Browse and compare all available property models')}
        actionButton={{
          label: t('models:compareSelected', 'Compare Selected'),
          onClick: handleCompare,
          icon: <CompareArrows />,
          tooltip: t('models:selectToCompare', 'Select 2 models to compare'),
          disabled: selected.length !== 2
        }}
      />
      <Typography variant="caption" sx={{ ml: 2, color: '#6c757d', mb: 4, display: 'block' }}>
        {t('models:selectToCompare', 'Select 2 models to compare')}
      </Typography>
      ...
      >
        ...
        {compareModels.length === 2 && (
          <ModelCustomizationCore
            ...
            labels={labels}
            confirmLabel={t('models:confirmComparison', 'Confirm Comparison')}
            onConfirm={result => {
